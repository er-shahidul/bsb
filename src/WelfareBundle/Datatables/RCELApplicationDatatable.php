...
/**
 * Class RCELApplicationDatatable
 *
 * @package WelfareBundle\Datatables
 */
class RCELApplicationDatatable extends BaseDatatable
{
    ...
    public function configureDataTable(array $options = array())
    {
        ...
        $this->ajax->set($this->getDefaultAjax(['url' => $this->router->generate('welfare_rcel_index')]));
        $this->setDefaultExportButtons([0,1,2,6,7]);

        $this->addActionButton('welfare_rcel_view', 'View', 'glyphicon-eye-open', ['id' => 'id']);

        if ($this->authorizationChecker->isGranted(['ROLE_DASB_CLERK'])) {
            $this->addActionButton('welfare_rcel_edit', 'Edit', 'glyphicon-edit', ['id' => 'id'], function($row) {
    ...
    {
        return 'WelfareBundle\Entity\RCELApplication';
    }
    ...
    {
        return 'rcelapplication_datatable';
    }
}
